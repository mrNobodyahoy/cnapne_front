...
export const generateGuidancePdf = (guidance: ReadTeacherGuidance) => {
  ...
  const primaryColor = "#217346";
  const grayColor = "#555555";
  const lightGrayColor = "#DDDDDD";

  const cnapneLogoPath = cnapneLogoMaior;
  ...
  doc.setFont("helvetica", "normal");
  doc.text(guidance.author?.fullName || "Sistema", margin + 50, currentY);
  currentY += lineHeight;

  doc.setFont("helvetica", "bold");
  doc.text("Especialidade:", margin, currentY);
  doc.setFont("helvetica", "normal");
  doc.text(guidance.author?.specialty || "N/A", margin + 50, currentY);
  currentY += lineHeight * 2;
